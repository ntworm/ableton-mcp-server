...
| **M0** overfit controlado | **feito em 2026-08-31.** Memorizou 64 exemplos: perda de treino `0,0463` contra limiar de `0,05`, com validação subindo, que é o comportamento pedido. 538 s numa thread de CPU | D1 |
| **M1** tiny 1% | **braço masked HVO feito em 2026-08-31**, três seeds, 4.047 passos cada, 1% do dataset. Os braços deterministic/retrieval, GrooVAE e event AR são o plano 5. Ver seção 20 | M0, P0 |
...
`[fato]` **Nota sobre a primeira tentativa do G4, guardada porque ensina mais que a aprovada.** O treino inicial usou BCE puro para o head de hit. O corpus tem hit em `5,51%` das células, ou 18,15 negativos por positivo, e BCE puro nessa proporção é minimizada prevendo silêncio: o modelo convergiu para `0,65%` de probabilidade média de hit e gerou `0,67` hits por barra. A perda de validação estava em `0,165` a `0,175` e caía suavemente nos três seeds — a curva parecia um treino saudável enquanto a saída era silêncio. É o risco 13 acontecendo, e só a cláusula de densidade do G4 pegou. Remédio: `pos_weight = 18,15` derivado da taxa medida, que é o que a seção 15.1 já prescrevia ao dizer "BCE ou focal loss conforme desbalanceamento". Nenhum limiar foi movido; a rodada reprovada está arquivada.

`[risco]` **Passar no G4 não significa que o modelo é musical, e a medição prova isso.** A distribuição por lane do modelo aprovado despeja `14,94` hits por janela em `hat_pedal` contra `2,38` do corpus, e produz **zero** `hat_closed` contra `4,99` do corpus — justamente a lane que o plano 2 existiu para recuperar. O G4 pergunta se o modelo está mudo, congelado ou inválido, e ele não está nenhum dos três. Plausibilidade por lane é métrica de musicalidade, já listada na seção 18.3, e pertence ao harness de avaliação do plano 5.
