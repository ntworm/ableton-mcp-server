...
def test_baseline_probe_names_equal_catalog() -> None:
    """The flattened probe map must cover the full catalog surface."""
    ...
    assert len(set(flattened)) == len(TOOL_CATALOG)
    assert len(flattened) == len(TOOL_CATALOG)  # every tool appears in exactly one group
...
def test_report_rejects_missing_catalog_rows() -> None:
    ...
    with pytest.raises(ValueError, match=f"{len(TOOL_CATALOG) - 1} tools are unclassified"):
        report.finish()
