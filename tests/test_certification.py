...
def test_baseline_probe_names_equal_catalog() -> None:
    """The flattened probe map must cover the full 75-tool baseline surface."""
    ...
    assert len(set(flattened)) == 75
    assert len(flattened) == 75  # every tool appears in exactly one group
...
def test_report_rejects_missing_catalog_rows() -> None:
    ...
    with pytest.raises(ValueError, match="74 tools are unclassified"):
        report.finish()
