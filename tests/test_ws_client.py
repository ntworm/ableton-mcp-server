...
async def test_ws_client_success() -> None:
    ...
    mock_connection.recv = AsyncMock(return_value=json.dumps({
        "jsonrpc": "2.0",
        "result": {"warping": True, "warp_mode": "complex"},
        "id": 1
    }))
    ...
    with patch("websockets.asyncio.client.connect", return_value=mock_connection) as mock_connect:
        ...
        mock_connect.assert_called_once_with("ws://127.0.0.1:9889", open_timeout=3.0, close_timeout=2.0)
...
async def test_ws_client_error_response() -> None:
    ...
    with patch("websockets.asyncio.client.connect", return_value=mock_connection):
        with pytest.raises(Exception, match="Clip is not an audio clip"):
